 ...
 */
package com.pardot.rhombus.driver;
 ...
 * priority. For example, if you wrap {@link PDCAwareRoundRobinPolicy} with this
 * token aware policy, replicas from remote data centers may only be
 * returned after all the host of the local data center.
 */
public class PTokenAwarePolicy implements LoadBalancingPolicy {
	private static final Logger logger = LoggerFactory.getLogger(PTokenAwarePolicy.class);
	 ...
	 * awareness.
	 */
	public PTokenAwarePolicy(LoadBalancingPolicy childPolicy, String datacenter) {
				...
				while (iter.hasNext()) {
					...
					if (host.getMonitor().isUp() && childPolicy.distance(host) == HostDistance.LOCAL) {
						if(!datacenter.equals(host.getDatacenter())) {
							logger.warn("Local datacenter {} returning host {} with datacenter {}", datacenter, host, host.getDatacenter());
						}
						return host;
					}
				}
